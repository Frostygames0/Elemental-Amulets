        ...
        private static ArgumentBuilder<CommandSource, ?> register() {
            return Commands.literal("give")
                    ...
                    .then(Commands.argument("player", EntityArgument.players())
                            .then(Commands.argument("amulet", ItemArgument.item()).suggests((ctx, builder) -> ISuggestionProvider.suggest(ModItems.AMULETS.get().stream().map(item -> item.getRegistryName().toString()), builder)).executes(ctx -> execute(ctx.getSource(), EntityArgument.getPlayers(ctx, "player"), ItemArgument.getItem(ctx, "amulet"), 1))
