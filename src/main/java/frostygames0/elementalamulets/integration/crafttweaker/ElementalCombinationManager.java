...
public class ElementalCombinationManager implements IRecipeManager {
     ...
     * If you want to replace recipe see {@link ElementalCombinationManager#replaceRecipe(String, IItemStack, IIngredient[], int, boolean)}
     *
     ...
     * @param combinationTime how long will it will combine
     *
    ...
    public void addRecipe(String name, IItemStack output, IIngredient[] ingredients, IItemStack elemental, int combinationTime, @ZenCodeType.OptionalBoolean boolean tagTransfer) {
        ...
        CraftTweakerAPI.apply(new ActionAddRecipe(this,
                new ElementalCombination(id, Arrays.stream(ingredients).map(IIngredient::asVanillaIngredient).collect(Collectors.toCollection(NonNullList::create)),
                        AmuletIngredient.fromStack(elemental.getInternal()), output.getInternal(), combinationTime, tagTransfer)));
    }
     ...
     * @docParam ingredients [<item:minecraft:gold_ingot>, <item:minecraft:gold_ingot>]
     * @docParam combinationTime 30
     * @see ElementalCombinationManager#addRecipe(String, IItemStack, IIngredient[], IItemStack, int, boolean)
     */
    @ZenCodeType.Method
    public void replaceRecipe(String name, IItemStack elemental, IIngredient[] ingredients, int combinationTime, @ZenCodeType.OptionalBoolean boolean tagTransfer) {
        /*
        ResourceLocation resourceLocation = new ResourceLocation(name);
        CraftTweakerAPI.apply(new ActionRemoveRecipeByName(this, resourceLocation));
        this.addRecipe(resourceLocation.getPath(), this.getRecipeByName(name).getOutput(), ingredients, elemental, combinationTime, tagTransfer);*/
        CraftTweakerAPI.logWarning("ElementalCombination.replaceRecipe currently is not working! Please replace recipe with usual ways");
    }
